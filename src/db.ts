...
export class DB {
  ...
  public column_names(indicator: string): Promise<any> {
    return this.db.any('SELECT * FROM $1~ LIMIT 1;', indicator);
  }

  // tslint:disable-next-line max-line-length
  public single_table(column_names: string[], indicator: string, entities?: string, start_year = '0', end_year = '9999', limit = '1000000', offset = '0'): Promise<any> {
    let entity_name = 'di_id';
    let entity_table = 'di_entity';
    if (column_names.indexOf('di_id') > -1) {
      entity_name = 'di_id';
      entity_table = 'di_entity';
    } else if (column_names.indexOf('to_di_id') > -1) {
      entity_name = 'to_di_id';
      entity_table = 'di_entity';
    } else if (column_names.indexOf('district_id') > -1) {
    ...
    if (column_names.indexOf('year') > -1) {
      if (entities != null) {
        const entities_arr = entities.split(',');
  ...
  multi_table(column_names: string[], indicators: string[], entities?: string, start_year = '0', end_year = '9999', limit = '1000000', offset = '0'): Promise<any> {
    const promise_arr = [];
  ...
  meta_data(): Promise<any> {
    return this.db.any('SELECT * FROM public.di_concept_in_dh;');
  }

  format_data(data: object[], format = 'json'): string {
    if (format === 'xml') {
      const options = { compact: true, ignoreComment: true };
      const data_obj = JSON.stringify({ record: data });
  ...
  format_error(error_data: { code: string }, format = 'json'): string {
    const data: any = {};
    // Known error codes
    ...
    if (format === 'xml') {
      const options = { compact: true, ignoreComment: true };
      const data_obj = JSON.stringify({ error: data });
